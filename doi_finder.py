...
def google_title_to_doi(title):
    ...
    try:
        ...
        pass
def google_doi(journal, volume, page, title, auth):
    # use only the first 3 words of the title
    title_s = " ".join(title.split()[:5])
    print(auth, title)
    browser.open("http://www.google.de/search?q={0} {1} {2} {3} {4}".format(
        journal, volume, page, title, auth).replace(" ", "%20"))
    assert browser.viewing_html()
    try:
        browser.follow_link(text_regex=r"(?i)^" + title_s + ".*")
        sourcecode = browser.response().get_data()
        doi = find_doi(sourcecode)
        return doi
    except:
        pass
